...
if (FALSE) {
  ...
  ## the following lines taken from viboud_funcs.r
  
  rm(list=ls())
  ## insert line here to set working directory to root folder of repo
  source("src/R/riley_funcs.r")
  source("src/R/viboud_funcs.r")
  source("src/R/walters_yan_funcs.R")
  library(ggplot2)
  library(deSolve)
  ...
  fluIliCountryData <- extract.incidence(
    dataflu$synd,
    minYear=2010,
    sel_iso3 = sel_iso,
    sel_ag = c("All"),
  ...
  # save(fluIliCountryData,file="~/fluIliCountryData.rda")

  ## new code starts here
  
  ## plot incidence for all years for a particular country (in this case Israel)
  all_incidence_plot <- plot_incidence_all(fluIliCountryData, "ISR")
  
  ## extract data for a single year and plot it
  incidence_data <- extract_incidence(fluIliCountryData, 
                                      country = "ISR", 
                                      year = 2016)
  
  single_year_incidence_plot <- plot_incidence(incidence_data)
  
  ## linear regression where we are currently at week 49 of the calendar year, 
  ## and we use data from the previous 4 weeks to predict the incidence
  ## 8 weeks ahead
  ...
  prediction_df <- extract_predicted_points(linear_regression_output, 
                                            ...
                                            weeks_ahead = 8,
                                            log_transform = TRUE)
  linear_regression_plot <- plot_incidence(incidence_data)
  
  ## calculate the proportion of predictions which are within 25% of the 
  ## observed values
  print(calc_prediction_accuracy(prediction_df))
  
  ## fit an SEIR model to the data.
  ## assumed parameter values:
  ## mean latent period = 1.6 days
  ## mean infectious period = 1 day
  ## population size = 8.5e6
  ## I_0 = 100 infectious individuals
  ## reporting rate = 0.006
  
  ## plot the likelihood for different values of R_0
  ## the likelihood is calculated using the data from our guess for the starting
  ## week of the epidemic, up to the current week.
  ## Assume Poisson-distributed error.
  current_week <- 49
  ...
  likelihood_profile_plot <- plot_likelihood_profile(likelihood_profile_output)
  
  ## fit value of R_0 to data
  R_0_min <- 1
  R_0_max <- 2
  ...
  prediction_df <- extract_predicted_points_seir(R_0, incidence_data, 
                                                 ...
                                                 weeks_ahead = 8)
  ## plot prediction
  SEIR_plot <- plot_incidence(prediction_df)
